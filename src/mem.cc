...
void
memFree(void *const p, const int type)
{
    assert(MemPools[type]);
...
void * const
memAllocString(size_t net_size, size_t * const gross_size)
{
...
memReallocBuf(void *oldbuf, size_t net_size, size_t * gross_size)
{
    ...
    assert(newbuf && "mem.cc::memReallocBuf::newbuf is empty");
    if (oldbuf) {
...
extern uint64_t _httpctx_c;
extern uint64_t _httpctx_d;
extern uint64_t _httpctx_k;
extern uint64_t _fwdsctx_c;
extern uint64_t _fwdsctx_d;
extern uint64_t _fwdstate_fails_;
extern t_HttpRmap _RQmap;
extern t_HttpRmap _RLmap;

const int Check_Trackers_age_limit_seconds = 600;
extern void Check_Titan_trackers_and_usage( std::ostream & a_os );
extern void Check_tracker_FwdState( std::ostream & a_os, uint32_t a_older_than_secs);
extern void Check_tracker_AccessLogEntry( std::ostream & a_os, uint32_t a_older_than_secs);
extern void Check_tracker_HttpRequest( std::ostream & a_os, uint32_t a_older_than_secs);

extern void get_HttpBody_life_stats(std::ostream & a_os);
extern void get_HttpHeader_life_stats(std::ostream & a_os);
extern void get_HttpReply_life_stats(std::ostream & a_s);
extern void get_MemBuf_Life_counts( uint64_t & a_c, uint64_t & a_d );
...
Mem::Report(std::ostream &stream)
{
   ...
   //Titax : trackers and counters to detect memory leaks
   Check_Titan_trackers_and_usage( stream, Check_Trackers_age_limit_seconds );
   Check_tracker_FwdState( stream, Check_Trackers_age_limit_seconds );
   Check_tracker_AccessLogEntry( stream, Check_Trackers_age_limit_seconds );
   Check_tracker_HttpRequest( stream, Check_Trackers_age_limit_seconds );
   get_HttpBody_life_stats( stream );
   get_HttpHeader_life_stats( stream );
   get_HttpReply_life_stats( stream );
   {
       uint64_t l_c;
       uint64_t l_d;
       get_MemBuf_Life_counts( l_c, l_d);
       stream << " memBuf: c = " << l_c << " d = " << l_d << "(" << (l_c - l_d) << ")\n";
   }

   stream << "\nActive FwdState: "<< _fwdsctx_c <<"/"<< _fwdsctx_d<<"/"<<(_fwdsctx_c-_fwdsctx_d) << "  (fails: " << _fwdstate_fails_ << ")";
   stream << "\nActive HttpRequests: "<< _httpctx_c <<"/"<< _httpctx_d<<"/" << (_httpctx_c - _httpctx_d) << "  k=" << _httpctx_k << "     RQmap: "<<_RQmap.size()<< " :\n";

   /**  This code does not show any leaking HttpRequest as it needs
    *   items to be added/ removed to /from RQmap (commented out)
    *   (An alternative is to use the instance_tracker on individual classes)
    * 
   stream << "Dangling HttpRequests: (older than 2 minutes)\n";

   std::chrono::time_point<std::chrono::system_clock> _now = std::chrono::system_clock::now();
   std::chrono::duration<double> elapsed_time;

   for (t_HttpRmap::iterator it=_RQmap.begin(); it!=_RQmap.end(); ++it){

      if (HttpRequest * const _h=static_cast<HttpRequest * const>(it->second)){         
         elapsed_time=_now-_h->ttag.timestamp_;

         if (elapsed_time.count()>120){

            stream <<">>> :"<< it->first << " => ";
            stream<< "::" <<_h->get_host() <<"|"<< (_h->pinnedConnection()?1:0)<<"|"<<(_h->clientConnectionManager.valid()?1:0)<<"|"<<(_h->persistent()?1:0)<< "::";

            if (_h->clientConnectionManager.valid()){
               if (Ssl::ServerBump *const serverBump =static_cast<Ssl::ServerBump *const>(_h->clientConnectionManager->serverBump()))
                  stream<<serverBump->_id<<"/"<<serverBump->_entry._id<< "::";
            };

            std::string _ts{_h->ttag.get_timestamp()};
            stream<<_ts<<"\n";

            stream<<"\t[cli_con]\n";
            if (_h->clientConnectionManager.valid()){
               Comm::ConnectionPointer & ccon=_h->clientConnectionManager->clientConnection;
               if (ccon!=NULL){
                     stream<<"\t\t - local=" << ccon->local<< "\n\t\t - remote=" << ccon->remote <<"\n\t\t - :FD " << ccon->fd<<'\n';
               }; 
            };

            stream<<"\t[LOCKS]:"<<_h->LockCount()<<"\n";
            if (_h->LockCount()){
               for (std::map<uint32_t,std::string>::iterator __lock=_h->_locks.begin(); __lock!=_h->_locks.end(); ++__lock){
                  stream <<"\t\t - :"<<__lock->first<<"|"<<__lock->second<<"\n";
               };
            };

            stream<<"\t[SNI]\n";
            std::string _sni=_h->get_sni();
            if (_sni.size()){
               stream<<"\t\t"<<_sni;
            };
            ...
            stream<<'\n';
            stream<<"\t[URI]\n";
            stream<<"\t\t"<<_h->get_canonical()<<"\n\n";
         };
      };
   };
       stream<<"----------------\n";
   
   for (t_HttpRmap::iterator it=_RLmap.begin(); it!=_RLmap.end(); ++it){

      if (HttpReply * const _h=static_cast<HttpReply * const>(it->second)){         
         {
