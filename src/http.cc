...
#define REFRESH_OVERRIDE(flag) 0
    ...
    case Http::scBadRequest:
        ...
        /* These responses can never be cached, some of them can be shared though*/
...
httpMakeVaryMark(HttpRequest * request, HttpReply const * reply)
{
    ...
    vary.clean();
    ary = reply->header.getList(HDR_X_ACCELERATOR_VARY);
    assembleVaryKey(vary, vstr, *request);
#endif
...
HttpStateData::processReplyHeader()
{
    ...
    const titan_v3::cfg::TCFGInfo & _ttncfg = titan_v3::GTAPE.ttncfg;

   if (_ttncfg.wtc_strip_some_headers){
      //TITAN: Strip some headers

      HttpHeader * const hdr=&newrep->header;
      const bool add_hacao=static_cast<bool>( request->headers_has(HDR_ORIGIN));

      for (HttpHeaderPos pos=0; pos < static_cast<ssize_t>(hdr->entries.size()); ++pos) {
         if (HttpHeaderEntry * const _e=static_cast<HttpHeaderEntry*const>(hdr->entries[pos])){
            switch (_e->id){
               case HDR_CSP:
               case HDR_CSP_lc:
               case HDR_CSP_RO:
               case HDR_X_CSP:
               case HDR_X_CSP_lc:
               case HDR_X_WEBKIT_CSP:
               case HDR_X_XSS_PROTECTION:
               case HDR_X_XSS_PROTECTION_lc:
               case HDR_X_FRAME_OPTIONS:
               case HDR_X_FRAME_OPTIONS_lc:{
                  hdr->entries[pos]=NULL;
                  delete _e;
                  continue;
               }break;
               //strip when needed
               case HDR_ACCESS_CONTROL_ALLOW_ORIGIN:{
                  if (add_hacao){
                     hdr->entries[pos]=NULL;
                     delete _e;
                     continue;
      ...
      //TITAN: Add some headers - but I'm not sure we need this at all

      //add when needed
      if (add_hacao) hdr->putStr(HDR_ACCESS_CONTROL_ALLOW_ORIGIN,"*");
      //TITAN 
   };
...
{
    // too late to read
    if (!Comm::IsConnOpen(serverConnection) || !fd_table[serverConnection->fd].flags.open || fd_table[serverConnection->fd].closing())
        return;
 ...
 */
void
HttpStateData::httpBuildRequestHeader(HttpRequest * const request,
...
HttpStateData::start()
{
   ...
   if (Comm::IsConnOpen(serverConnection) && (!fd_table[serverConnection->fd].flags.open || fd_table[serverConnection->fd].closing())){
      //DEV::maybe add some logic/counter - and if this situation occurs too often we could actually die/assert
      debugs(11, DBG_IMPORTANT, "httpStart: aborted(0)");
      this->abortTransaction("httpStart: aborted(0)");
      return;
   };

   if (!sendRequest()) {
     debugs(11, 3, "httpStart: aborted(1)");
