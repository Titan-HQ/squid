...
void
allocate_new_swapdir(SquidConfig::_cacheSwap * const swap)
{
   if (swap){
      if (swap->swapDirs == NULL) {
         (void)(!swap->n_allocated && (swap->n_allocated = 4));
         swap->swapDirs=static_cast<SwapDir::Pointer *const>(xcalloc(swap->n_allocated, sizeof(SwapDir::Pointer)));
         assert(swap->swapDirs!=NULL);
      };
      if (swap->n_allocated == swap->n_configured){
         if (SwapDir::Pointer *const tmp = static_cast<SwapDir::Pointer *const>(xcalloc((swap->n_allocated <<= 1), sizeof(SwapDir::Pointer)))){
            (void)memcpy(tmp, swap->swapDirs, swap->n_configured * sizeof(SwapDir *));
            xfree(swap->swapDirs);
            swap->swapDirs = tmp;
         } else {
            assert(0 && "allocate_new_swapdir::xcalloc");
         };
      };
      return;
   };
   assert(0 && "allocate_new_swapdir");
}

void
free_cachedir(SquidConfig::_cacheSwap * const swap)
{
    int i;
