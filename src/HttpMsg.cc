...
void HttpMsg::lock(const char * const _place, const uint32_t _no  ) const {

/*
   std::string _line("L:");
   _line+= std::to_string(this->_id);
   _line+=":";
   _line+=_place;
   _line+=":";
   _line+=std::to_string(_no);
   _line+=":";
*/
   Lock::lock(_place,_no);

/*   
   _line+=std::to_string(this->LockCount());
   _locks[_locks.size()]=_line;
 */

   WTRACE_MSG(">>"+_line);
};

uint32_t HttpMsg::unlock(const char * const _place,const uint32_t _no) const {
...
*/   
   const uint32_t _r=Lock::unlock(_place,_no);
...
void HttpMsg::trace(const char * const _place,const uint32_t _no,const std::string _msg) const {
/*
    std::string _line("T:");
   _line+=std::to_string(this->_id);
   _line+=":";
