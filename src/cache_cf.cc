    ...
    if (geteuid() == 0) {
        if (NULL != Config.effectiveUser) {
         ...
         if (pwd->pw_dir && *pwd->pw_dir) {
            ...
            // 20151204 - not anymore!!!
            static SBuf lastDir;
            if (lastDir.isEmpty() || lastDir.cmp(pwd->pw_dir) != 0) {
               lastDir = pwd->pw_dir;
               int len = strlen(pwd->pw_dir) + 6;
               char _env_str[len+1];
               (void)snprintf(_env_str, len, "HOME=%s", pwd->pw_dir);

               if (env_str){
                  if (0!=strcmp(_env_str,env_str)){
                     free(env_str);
                     env_str=0;
                  };
               };
               ...
               if (!env_str){
                  env_str = (char *)xcalloc(len, 1);
                  (void)memcpy(env_str,_env_str,len);
               };
               (void)putenv(env_str);
            };
         };
...
dump_cachedir(StoreEntry * entry, const char *name, SquidConfig::_cacheSwap swap)
{
    ...
    for (int i = 0; i < swap.n_configured; ++i) {
        try{
           if (SwapDir * const s = dynamic_cast<SwapDir *const>(swap.swapDirs[i].getRaw())){
...
static void
free_authparam(Auth::ConfigVector * const cfg)
{
    /* Wipe the Auth globals and Detach/Destruct component config + state. */

   if (cfg){
      Auth::ConfigVector::iterator _item=cfg->begin();
      while(_item!=cfg->end()) {
         delete(*_item);
         ++_item;
      };
...
static void
parse_cachedir(SquidConfig::_cacheSwap * const swap)
{
   ...
   if (swap){
      ...
      SwapDir * _x=NULL;
      for (int i = 0; i < swap->n_configured; ++i) {
            if (!swap->swapDirs[i].getRaw() || !(_x=dynamic_cast<SwapDir *>(swap->swapDirs[i].getRaw())) || (strcasecmp(path_str, _x->path) != 0)) continue;
            ...
            */
            ...
            SwapDir::Pointer sd=swap->swapDirs[i];
      ...
      if (swap->swapDirs[swap->n_configured]!=NULL){
         /* parse the FS parameters and options */
         swap->swapDirs[swap->n_configured]->parse(swap->n_configured, path_str);
         ++swap->n_configured;         
      };
   };
}
...
void
parse_int(int *const var)
{
   (void)(var && (*var = GetInteger()));
}

static void
free_int(int *const var)
{
   (void)(var && (*var = 0));
}
...
void
parse_onoff(int *const var)
{
    if (const char *const token = (const char*const)ConfigParser::NextToken()){

#define _T1       "on"
#define _T1sz     (sizeof(_T1)-1)
#define _T2       "enable"
#define _T2sz     (sizeof(_T2)-1)
#define _T3       "off"
#define _T3sz     (sizeof(_T3)-1)
#define _T4       "disable"
#define _T4sz     (sizeof(_T4)-1)

       if (!strncmp(token, _T1,_T1sz)) {
           *var = 1;
           return;
       } else if (!strncmp(token,_T2,_T2sz)) {
           debugs(0, DBG_PARSE_NOTE(DBG_IMPORTANT), "WARNING: 'enable' is deprecated. Please update to use 'on'.");
           *var = 1;
           return;
       } else if (!strncmp(token, _T3,_T3sz)) {
           *var = 0;
           return;
       } else if (!strncmp(token, _T4,_T4sz)) {
...
void
ConfigParser::ParseWordList(wordlist ** const list)
{
    parse_wordlist(list);
}

void
parse_wordlist(wordlist ** const list)
{
   while (const char * const token = ConfigParser::NextQuotedToken()) wordlistAdd(list, token);
...
static void
parsePortSpecification(const AnyP::PortCfgPointer &s, char *const token)
{
    char *host = nullptr;
...
static void
parse_port_option(AnyP::PortCfgPointer &s, const char *const token)
{
    /* modes first */
...
void
add_http_port(char *const portspec)
{
...
void
requirePathnameExists(const char *const name, const char *const path)
{
   if (path && name){
      char pathbuf[BUFSIZ];
      const char * const _path=((Config.chroot_dir && (geteuid() == 0) && snprintf(pathbuf, BUFSIZ, "%s/%s", Config.chroot_dir, path)>0)?pathbuf:path);
      struct stat sb;
      if (_path && stat(_path, &sb) < 0) {
        debugs(0, DBG_CRITICAL, (opt_parse_cfg_only?"FATAL ":"") << "ERROR: " << name << " " << _path << ": " << xstrerror());
        ...
        if (opt_send_signal == -1 || opt_send_signal == SIGHUP)
            fatalf("%s %s: %s", name, _path, xstrerror());
      };
   };
}
