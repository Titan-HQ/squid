...
StoreEntry::delayAwareRead(const Comm::ConnectionPointer &conn, char *buf, int len, AsyncCall::Pointer callback)
{
    ...
    if (-1==conn->fd || !fd_table[conn->fd].flags.open || fd_table[conn->fd].closing()) {
...
void
destroyStoreEntry(void *const data)
{
    debugs(20, 3, HERE << "destroyStoreEntry: destroying " <<  data);

   if (hash_link * const _hlink=static_cast<hash_link *const>(data)){
      if (StoreEntry *const e = static_cast<StoreEntry *const>(_hlink)){
         ...
         if (e->swap_filen >= 0 && !shutting_down) {
            SwapDir::Pointer _eptr=e->store();
            if (_eptr!=NULL){
               try{
                  SwapDir & sd = dynamic_cast<SwapDir&>(*_eptr);
                  sd.disconnect(*e);
               } catch( std::bad_cast & l_e ) {
                   debugs( 20,1, "destroyStoreEntry() no disconnect because of exception : " << l_e.what() );
               }
            };
         };
...
{
    if ((key && !EBIT_TEST(flags, KEY_PRIVATE)) || !mem_obj)
...
void
storeAppendPrintf(StoreEntry * const e, const char * const fmt,...)
{
    va_list args;
    va_start(args, fmt);
...
void
storeAppendVPrintf(StoreEntry * const e, const char *const fmt, va_list vargs)
{
   if (e && fmt){
