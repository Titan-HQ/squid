...
public:
     ...
     */
    HttpReply *BuildHttpReply(const bool intercept_login=false);

    /// set error type-specific detail code
    void detailError(int dCode) {detailCode = dCode;}


    friend std::ostream& operator<<(std::ostream& out, ErrorState * obj);
...
public:
    ...
    char blocked_reason[513]; /* Titax ???? */
    char safe_blocked_reason[513]; /* Titax ???? */
    char* utf8_uri; //????
 ...
 */
void errorSend(const Comm::ConnectionPointer &conn, ErrorState *const err);
 ...
 *
 ...
 \param err     This object is destroyed after use in this function.
 */
void errorAppendEntry(StoreEntry *const entry, ErrorState *const err);
