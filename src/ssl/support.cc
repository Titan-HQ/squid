...
/// \ingroup ServerProtocolSSLInternal
long
Ssl::parse_options(const char *const options)
{

    if (!options) return 0;
    char *const tmp = xstrdup(options);
    char *option = strtok(tmp, ":,");
    long op = 0;
    while (option) {
...
/// \ingroup ServerProtocolSSLInternal
long
Ssl::parse_flags(const char *const flags)
{
    if (!flags)
        return 0;

    char *const tmp= xstrdup(flags);
...
STACK_OF(X509_CRL) *const
Ssl::loadCrl(const char *const CRLFile, long &flags)
{
    BIO *const in = BIO_new_file(CRLFile, "r");
    if (!in) {
        debugs(83, 2, "WARNING: Failed to open CRL file '" << CRLFile << "'");
        return NULL;
    }

    STACK_OF(X509_CRL) *const CRLs = sk_X509_CRL_new_null();
    if (!CRLs) {
...
sslCreateServerContext(AnyP::PortCfg &port)
{
    ...
    if (SSL_CTX *const sslContext= SSL_CTX_new(port.contextMethod)){
...
int Ssl::OpenSSLtoSquidSSLVersion(const int sslVersion)
{
...
SSL_METHOD * const
#else
const SSL_METHOD * const
#endif
Ssl::method(const int version)
{
    switch (version) {
...
const SSL_METHOD * const
Ssl::serverMethod(const int version)
{
    switch (version) {
...
static int
ssl_next_proto_cb(SSL *const s, unsigned char **const out, unsigned char *const outlen, const unsigned char *const in,const unsigned int inlen, void *const arg)
{
...
sslCreateClientContext(const char *certfile, const char *keyfile, int version, const char *cipher, const char *options, const char *flags, const char *CAfile, const char *CApath, const char *CRLfile)
{
    ...
    if (SSL_CTX * const sslContext = SSL_CTX_new(method)){
...
/// \ingroup ServerProtocolSSLInternal
int
ssl_read_method(const int fd, char *const buf,const  int len)
{
   if (-1!=fd && buf && len>0){
      if (SSL *const ssl = fd_table[fd].ssl){
         ...
         const int i = SSL_read(ssl, buf, len);
	if (i > 0) {
...
/// \ingroup ServerProtocolSSLInternal
int
ssl_write_method(const int fd, const char *const buf, const int len)
{
   if (-1!=fd && buf && len>0){
      if (SSL *const ssl = fd_table[fd].ssl){
         if (!SSL_is_init_finished(ssl)) {
...
void
ssl_shutdown_method(SSL *const ssl)
{
    SSL_shutdown(ssl);
}
...
/// \ingroup ServerProtocolSSLInternal
static bool
ssl_get_attribute(X509_NAME *const name, const char *const attribute_name, char * const _out, const uint64_t _osz)
{

    if (name && attribute_name && _out && _osz>0){

       *_out=0;

       if (strcmp(attribute_name, "DN") == 0) {
           X509_NAME_oneline(name, _out, _osz);
           return (*_out?true:false);
       }
       const int nid = OBJ_txt2nid(attribute_name);
       ...
       X509_NAME_get_text_by_NID(name, nid, _out, _osz);
       return (*_out?true:false);
    };
    return false;
}
...
/// \ingroup ServerProtocolSSLInternal
bool
Ssl::GetX509UserAttribute(X509 *const cert, const char *const attribute_name,char * const _out, const uint64_t _osz)
{
    if (cert && attribute_name && _out && _osz)
    if (X509_NAME *const name= X509_get_subject_name(cert))
       return ssl_get_attribute(name, attribute_name,_out,_osz);

    return (false);
}

bool
Ssl::GetX509Fingerprint(X509 *const cert, const char *const,char * const _out, const uint64_t _osz)
{
   if (cert &&_out && _osz){
      unsigned int n;
      unsigned char md[EVP_MAX_MD_SIZE];
      if ( X509_digest(cert, EVP_sha1(), md, &n) && ( (3 * n + 1) < _osz)){
         char *s = _out;
         for (unsigned int i=0; i < n; ++i, s += 3) {
             const char term = (i + 1 < n) ? ':' : '\0';
             (void)snprintf(s, 4, "%02X%c", md[i], term);
         };
         //FIXME::add some more testing to be sure
         return (true);
      };
   };
   return false;
...
/// \ingroup ServerProtocolSSLInternal
bool
Ssl::GetX509CAAttribute(X509 *const  cert, const char *const attribute_name,char *const _out,const uint64_t _osz  )
{
   if (cert && attribute_name && _out && _osz){
      if (X509_NAME *const name= X509_get_issuer_name(cert))
         return ssl_get_attribute(name, attribute_name,_out,_osz);
   };
   return false;
...
bool sslGetUserAttribute(SSL *const ssl, const char *const attribute_name,char * const _out, const uint64_t _osz)
{
    if (ssl && attribute_name && _out && _osz)
    if (X509 *const cert = SSL_get_peer_certificate(ssl)){
       if (Ssl::GetX509UserAttribute(cert, attribute_name,_out, _osz)){
          X509_free(cert);
          return true;
       };
       X509_free(cert);
...
bool sslGetCAAttribute(SSL *const ssl, const char *const attribute_name,char * const _out, const uint64_t _osz)
{
   if (ssl && attribute_name && _out &&_osz){
      if (X509 *const cert = SSL_get_peer_certificate(ssl)){
         return Ssl::GetX509CAAttribute(cert, attribute_name,_out,_osz);
      };
   };
   return false;
}

bool sslGetUserEmail(SSL *const ssl,char * const _out, const uint64_t _osz){

    return sslGetUserAttribute(ssl, "emailAddress",_out,_osz);
}

bool sslGetUserCertificatePEM(SSL *const ssl,char ** const _out,  uint64_t *const _osz){
   ...
   if (ssl && _out && _osz){
      ...
      assert(mem && "sslGetUserCertificatePEM->mem");

      PEM_write_bio_X509(mem, cert);
      char *ptr=0;

      if (const long len = BIO_get_mem_data(mem, &ptr)){

          if (*_out && *_osz>=(len + 1)){
             (void)memmove(*_out, ptr, len);
             *_out[len] = '\0';
          } else {
             if (*_out) free_const(*_out);
             *_osz=len + 1;
             *_out= (char *const)xmalloc(*_osz);
             (void)memcpy(*_out, ptr, len);
             *_out[len] = '\0';
          };
...
bool
sslGetUserCertificateChainPEM(SSL *const ssl,char ** const _out,  uint64_t *const _osz)
{

   if (ssl && _out && _osz){

      STACK_OF(X509) *const chain = SSL_get_peer_cert_chain(ssl);

      if (!chain)
        return sslGetUserCertificatePEM(ssl,_out,_osz);

      BIO *const mem = BIO_new(BIO_s_mem());
      assert(mem && "sslGetUserCertificateChainPEM->mem");

      for (uint32_t i = 0; i < sk_X509_num(chain); ++i) {
         X509 *const cert = sk_X509_value(chain, i);
         PEM_write_bio_X509(mem, cert);
      }
...
Ssl::ContextMethod
Ssl::contextMethod(const int version)
{
    Ssl::ContextMethod method;
...
SSL_CTX * const
Ssl::generateSslContextUsingPkeyAndCertFromMemory(const char *const data, AnyP::PortCfg &port)
{
...
bool
Ssl::configureSSL(SSL *const ssl, CertificateProperties const &properties, AnyP::PortCfg &port)
{
    Ssl::X509_Pointer cert;
    Ssl::EVP_PKEY_Pointer pkey;
    if (!generateSslCertificate(cert, pkey, properties))
...
bool Ssl::verifySslCertificate(SSL_CTX * const sslContext, CertificateProperties const &properties)
{
    // SSL_get_certificate is buggy in openssl versions 1.0.1d and 1.0.1e
    // Try to retrieve certificate directly from SSL_CTX object
#if SQUID_USE_SSLGETCERTIFICATE_HACK
    X509 ***pCert = (X509 ***)sslContext->cert;
    X509 * const cert = pCert && *pCert ? **pCert : NULL;
#elif SQUID_SSLGETCERTIFICATE_BUGGY
    X509 * const cert = NULL;
    assert(0);
#else
    // Temporary ssl for getting X509 certificate from SSL_CTX.
    Ssl::SSL_Pointer ssl(SSL_new(sslContext));
    X509 * const cert = SSL_get_certificate(ssl.get());
#endif
    if (!cert)
        return false;
    const ASN1_TIME * const time_notBefore = X509_get_notBefore(cert);
    const ASN1_TIME * const  time_notAfter = X509_get_notAfter(cert);
...
bool
Ssl::setClientSNI(SSL *const ssl, const char *const fqdn)
{
...
void Ssl::addChainToSslContext(SSL_CTX *const sslContext, STACK_OF(X509) *const  chain)
{
    if (!chain)
        return;
...
bool Ssl::generateUntrustedCert(X509_Pointer &untrustedCert, EVP_PKEY_Pointer &untrustedPkey, X509_Pointer const  &cert, EVP_PKEY_Pointer const & pkey)
{
    ...
    char _v[1024]={};
    if (CommonHostName(cert.get(),_v,sizeof(_v))) {
        certProperties.commonName = "Not trusted by \"";
        certProperties.commonName += _v;
        certProperties.commonName += "\"";
    } else if (getOrganization(cert.get(),_v,sizeof(_v))) {
        certProperties.commonName =  "Not trusted by \"";
        certProperties.commonName += _v;
        certProperties.commonName += "\"";
    } else
...
SSL *const
SslCreate(SSL_CTX *const sslContext, const int fd, Ssl::Bio::Type type, const char *const squidCtx)
{
    ...
    int errCode = 0;
    if (SSL *const ssl = SSL_new(sslContext)) {
        // without BIO, we would call SSL_set_fd(ssl, fd) instead
        if (BIO *const bio = Ssl::Bio::Create(fd, type)) {
...
SSL *const
Ssl::CreateClient(SSL_CTX *const sslContext, const int fd, const char *const squidCtx)
{
    return SslCreate(sslContext, fd, Ssl::Bio::BIO_TO_SERVER, squidCtx);
}

SSL *const
Ssl::CreateServer(SSL_CTX *const sslContext, const int fd, const char *const squidCtx)
{
    return SslCreate(sslContext, fd, Ssl::Bio::BIO_TO_CLIENT, squidCtx);
}

Ssl::CertError::CertError(ssl_error_t anErr, X509 *const aCert): code(anErr)
{
...
static int
store_session_cb(SSL *const ssl, SSL_SESSION *const session)
{
    if (!SslSessionCache)
        return 0;
    ...
    unsigned char *const id = session->session_id;
    const unsigned int idlen = session->session_id_length;
    ...
    memcpy(key, id, idlen);
    int pos;
    if (Ipc::MemMap::Slot *const slotW = SslSessionCache->openForWriting((const cache_key*const)key, pos)){
...
static void
remove_session_cb(SSL_CTX *const, SSL_SESSION *const sessionID)
{
    if (!SslSessionCache)
        return ;
    ...
    int pos;
    if (Ipc::MemMap::Slot const *const slot = SslSessionCache->openForReading((const cache_key*const)sessionID, pos)){
...
static SSL_SESSION *
get_session_cb(SSL *const , unsigned char *const sessionID, const int len, int * const copy)
{
    if (!SslSessionCache)
        return NULL;

    SSL_SESSION *session = NULL;
    const unsigned int *const p=(const unsigned int *const)sessionID;
...
static void
setSessionCallbacks(SSL_CTX *const ctx)
{
    if (SslSessionCache) {
