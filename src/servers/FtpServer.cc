...
bool
Ftp::Server::setReply(const int code, const char * const msg)
{
   ClientSocketContext::Pointer context = getCurrentContext();
   if (context!=NULL)
      if (ClientHttpRequest *const http = context->http){
         if (http && http->storeEntry()){
            HttpReply *const reply = Ftp::HttpReplyWrapper(code, msg, Http::scNoContent, 0);
            LOCAL_ARRAY(char, buf, MAX_URL);
            if (urlCanonicalClean(http->request,buf,MAX_URL)){
               setLogUri(http, buf);
               if (clientStreamNode *const node = context->getClientReplyContext()){
                  try{
                    ...
                    debugs( 33,1, "Ftp::Server::setReply() exception : " << l_e.what());
                    return false;
                  };
               };
            };
