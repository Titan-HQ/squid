...
Http::Server::start()
{
    ...
    if (Comm::IsConnOpen(clientConnection) && (!fd_table[clientConnection->fd].flags.open || fd_table[clientConnection->fd].closing())){
       //DEV::maybe add some logic/counter - and if this situation occurs too often we could actually die/assert
       debugs(11, DBG_IMPORTANT, "Http::Server::start:aborted");
