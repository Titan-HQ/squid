...
SSL_CTX * const sslCreateServerContext(AnyP::PortCfg &port) STUB_RETVAL(NULL)
SSL_CTX * const sslCreateClientContext(const char *certfile, const char *keyfile, int version, const char *cipher, const char *options, const char *flags, const char *CAfile, const char *CApath, const char *CRLfile) STUB_RETVAL(NULL)
...
STACK_OF(X509_CRL) * const loadCrl(const char *CRLFile, long &flags) STUB_RETVAL(NULL)
...
SSL_CTX * const generateSslContext(CertificateProperties const &properties, AnyP::PortCfg &port) STUB_RETVAL(NULL)
bool verifySslCertificate(SSL_CTX * sslContext,  CertificateProperties const &properties) STUB_RETVAL(false)
SSL_CTX * const generateSslContextUsingPkeyAndCertFromMemory(const char * data, AnyP::PortCfg &port) STUB_RETVAL(NULL)
