...
static void peerSelectDnsResults(const ipcache_addrs * const ia, const DnsLookupDetails &details, void *data);
static void peerSelectDnsResults_ex(const ipcache_addrs * const ia, const DnsLookupDetails &details, void *data,const uint32_t _port=0);
           ...
           void *callback_data)
{
    ...
    ps_state * psstate =  new ps_state;    // TODO: use std::unique_ptr<T> throughout
...
static 
void
peerSelectDnsPaths(ps_state * const psstate)
{
   if (psstate){
      ...
      HttpRequest *const req = psstate->request;
      const bool isIntercepted = !req->flags.redirected &&
                                 (req->flags.intercepted || req->flags.interceptTproxy);
      const bool useOriginalDst = !req->flags.ttn_client_dst_passthru &&
                                  (Config.onoff.client_dst_passthru || !req->flags.hostVerified);
      const bool choseDirect = fs && fs->code == HIER_DIRECT;

      const bool local=req->flags.ttn_is_local_request;
      
      const bool target=req->is_target_server();

      if (target){
         if (local && fs)
         {
            const cfg::TCFGInfo & _ttncfg = GTAPE.ttncfg;
            std::string l_bp_str = titan_v3::cidr::factory::to_string( _ttncfg.bp_backed_http.ip );
            if ( ! l_bp_str.empty() ) 
            {
                const ipcache_addrs * const addrs = ipcacheCheckNumeric( l_bp_str.c_str() );
               if ( addrs )
               {
                  const DnsLookupDetails details(NULL, -1); // no error, no lookup
                  peerSelectDnsResults_ex(addrs, details, psstate, req->get_port());
                  return;
               };
            };
         };
         ...
         if (isIntercepted && useOriginalDst && choseDirect) {
            ...
            if (ConnStateData * const _csd=static_cast<ConnStateData * const>(req->clientConnectionManager.valid())){
               // construct a "result" adding the ORIGINAL_DST to the set instead of DIRECT
               Comm::ConnectionPointer p = new Comm::Connection();
               p->remote = _csd->clientConnection->local;
     ...
     /* if (*isIntercepted && useOriginalDst && choseDirect) {
      ...
      if (fs && psstate->paths->size() < (unsigned int)Config.forward_max_tries) {
          ...
          const char * const host = fs->_peer ? fs->_peer->host : psstate->request->GetHost();
      ...
      PSC * const callback = psstate->callback;
   ...
   assert( 0 && "peerSelectDnsPaths");
}


static void peerSelectDnsResults(const ipcache_addrs * const ia, const DnsLookupDetails &details, void *data){
   peerSelectDnsResults_ex(ia,details,data,0);
};

static void
peerSelectDnsResults_ex(const ipcache_addrs *const ia, const DnsLookupDetails &details, void *data,const uint32_t _port)
{
    ps_state *psstate = (ps_state *)data;
    ...
    FwdServer * const fs = psstate->servers;
    if (ia != NULL) {
...
static void
peerSelectFoo(ps_state * const ps)
{
   if (ps){
