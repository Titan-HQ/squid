...
AnyP::ProtocolType urlParseProtocol(const char * const , const char *const e = NULL);
void urlInitialize(void);
HttpRequest *const urlParse(const HttpRequestMethod&, char *const , HttpRequest *const request = NULL);
const char *const urlCanonical(HttpRequest *const);
const char *const urlCanonicalEx(HttpRequest *const, size_t *const);
std::string urlCanonicalStr(HttpRequest *const);
bool urlCanonicalClean(const HttpRequest *const, char * const, const uint32_t);
bool urlCanonicalFakeHttps(const HttpRequest *const  request,char *const _out, const uint32_t _sz);
bool urlIsRelative(const char *const);
char *const urlMakeAbsolute(const HttpRequest *const, const char *const);
...
int matchDomainName(const char *const host, const char *const domain, uint flags = mdnNone);
int urlCheckRequest(const HttpRequest *const );
int urlDefaultPort(const AnyP::ProtocolType p);
char *const urlHostname(const char *const url);
