...
static char *const makeExternalAclKey(ACLFilledChecklist *const  ch, external_acl_data *const  acl_data);
static void external_acl_cache_delete(external_acl * const  def, const ExternalACLEntryPointer &entry);
static int external_acl_entry_expired(external_acl * const  def, const ExternalACLEntryPointer &entry);
static int external_acl_grace_expired(external_acl * const  def, const ExternalACLEntryPointer &entry);
static void external_acl_cache_touch(external_acl *const def, const ExternalACLEntryPointer &entry);
static ExternalACLEntryPointer external_acl_cache_add(external_acl * const def, const char *const key, ExternalACLEntryData const &data);
...
external_acl::add(const ExternalACLEntryPointer &anEntry)
{
    ...
    e->lock(__FUNCTION__,__LINE__); //cbdataReference(e); // lock it on behalf of the hash
    ++cache_entries;
}
...
aclMatchExternal(external_acl_data *acl, ACLFilledChecklist *ch)
{
    ...
    if (entry != NULL) {
        if (entry->def == acl->def) {
            ...
            const char *const key = makeExternalAclKey(ch, acl);
            if (!key)
...
static void
external_acl_cache_touch(external_acl * const def, const ExternalACLEntryPointer &entry)
{
...
external_acl_ssl_get_user_attribute(const ACLFilledChecklist &ch, const char *attr)
{
    if (ch.conn() != NULL && Comm::IsConnOpen(ch.conn()->clientConnection)) {
        if (SSL *ssl = fd_table[ch.conn()->clientConnection->fd].ssl) {
            ...
            (void)zm(buffer,sizeof(buffer));
            return (sslGetUserAttribute(ssl, attr, buffer, sizeof(buffer))) ? buffer : NULL;
        }
    }
    return NULL;
}
...
static char * const
makeExternalAclKey(ACLFilledChecklist * const ch, external_acl_data *const  acl_data)
{
    static MemBuf mb;
    char buf[256];
    int first = 1;
    ...
    static char * _sslGetUserCertificate_mem=0;
    static uint64_t _sslGetUserCertificate_mem_sz=0;
    ...
    for (external_acl_format::Pointer format = acl_data->def->format; format != NULL; format = format->next) {
        ...
        case Format::LFT_EXT_ACL_CLIENT_EUI48:
            if (ConnStateData * const _csd=static_cast<ConnStateData * const>(request->clientConnectionManager.valid())){
               Comm::ConnectionPointer &_cc=_csd->clientConnection;
               if (_cc!=NULL && _cc->remoteEui48.encode(buf, sizeof(buf))){
                str = buf;
               };
            };
            break;
        ...
        case Format::LFT_EXT_ACL_CLIENT_EUI64:
            if (ConnStateData * const _csd=static_cast<ConnStateData * const>(request->clientConnectionManager.valid())){
               Comm::ConnectionPointer &_cc=_csd->clientConnection;
               if (_cc!=NULL && _cc->remoteEui64.encode(buf, sizeof(buf))){
                str = buf;
               };
            };
            break;
        ...
        case Format::LFT_EXT_ACL_USER_CERT_RAW:
            ...
            if (ch->conn() != NULL && Comm::IsConnOpen(ch->conn()->clientConnection)) {
                if (SSL *const ssl = fd_table[ch->conn()->clientConnection->fd].ssl){
                    str = (sslGetUserCertificatePEM(ssl,&_sslGetUserCertificate_mem,&_sslGetUserCertificate_mem_sz)?_sslGetUserCertificate_mem:NULL);
                }
            }
        ...
        case Format::LFT_EXT_ACL_USER_CERTCHAIN_RAW:
            ...
            if (ch->conn() != NULL && Comm::IsConnOpen(ch->conn()->clientConnection)) {
                if (SSL *const ssl = fd_table[ch->conn()->clientConnection->fd].ssl){
                    str = (sslGetUserCertificateChainPEM(ssl,&_sslGetUserCertificate_mem,&_sslGetUserCertificate_mem_sz)?_sslGetUserCertificate_mem:NULL);
                };
            }
        ...
        case Format::LFT_EXT_ACL_USER_CA_CERT:
            ...
            if (ch->conn() != NULL && Comm::IsConnOpen(ch->conn()->clientConnection)) {
                if (SSL * const ssl = fd_table[ch->conn()->clientConnection->fd].ssl){
                   if (sslGetCAAttribute(ssl, format->header,_buff,sizeof(_buff)))
                      str =_buff;
                }
            }
        ...
        case Format::LFT_SSL_CLIENT_SNI:
            if (ch->conn() != NULL) {
                if (Ssl::ServerBump * const srvBump = ch->conn()->serverBump()) {
        ...
        case Format::LFT_SSL_SERVER_CERT_ISSUER: {
            ...
            if (serverCert) {
                if (format->type == Format::LFT_SSL_SERVER_CERT_SUBJECT)
                    ...
                    str = (Ssl::GetX509UserAttribute(serverCert, "DN",_buff,sizeof(_buff))?_buff:NULL);
                else
                    str = (Ssl::GetX509CAAttribute(serverCert, "DN",_buff,sizeof(_buff))?_buff:NULL);
            }
            break;
        }
...
static int
external_acl_entry_expired(external_acl *const  def, const ExternalACLEntryPointer &entry)
{
...
static int
external_acl_grace_expired(external_acl *const  def, const ExternalACLEntryPointer &entry)
{
    if (def->cache_size <= 0 || entry->result == ACCESS_DUNNO)
        return 1;
...
external_acl_cache_add(external_acl * const def, const char *const  key, ExternalACLEntryData const & data)
{
...
static void
external_acl_cache_delete(external_acl *const  def, const ExternalACLEntryPointer &entry)
{
    assert(entry != NULL);
    ...
    e->unlock(__FUNCTION__,__LINE__); // unlock on behalf of the hash
    def->cache_entries -= 1;
}
