...
load_dict(FILE * FH) {
    ...
    struct ip_user_dict * const first_entry = static_cast<struct ip_user_dict* const>(xmalloc(sizeof(struct ip_user_dict)));    /* the head of the linked list */

    struct ip_user_dict *current_entry=first_entry; /* the structure used to store data */

    char line[DICT_BUFFER_SIZE]; /* the buffer for the lines read from the dict file */
    ...
    while (fgets(line, sizeof(line), FH) != NULL) {
        ...
        if (strtok(line, "\t ") != NULL) {
            ...
            if ((cp = strtok (line, "/")) != NULL) {
                ...
                const char *const tmpbuf=cp;  /* for the address before the bitwise AND */
                cp = strtok (NULL, "/");
                if (cp != NULL) {
